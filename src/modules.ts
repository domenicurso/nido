...
export async function loadModules(): Promise<Registry<ModuleDefinition>> {
  const directories = await readdir(MODULES_DIR, { withFileTypes: true });
  const all = await Promise.all(
    directories
      ...
      .map(async (entry) => {
        ...
        return parseModule(document, configPath, directory);
      }),
  );
  ...
  return {
    all,
    byId: new Map(all.map((module) => [module.id, module])),
  };
}
...
function parseModule(
  ...
  return {
    ...
    incompatibilities: readStringArray(
      record.incompatibilities,
      `${configPath} incompatibilities`,
      [],
    ),
    label: readString(record.label, `${configPath} label`),
    prompts: readPrompts(record.prompts, configPath),
    supportedTemplates: readStringArray(
      record.supportedTemplates,
      `${configPath} supportedTemplates`,
      [],
    ),
  };
}
